...
for date,cat,img,path,sha,size in ADD:
 if (date,img) not in seen:
  rows.append({'include':'1','data_date':date,'category':cat,'status':'verified','source_path':path,'source_image':img,'sha256':sha,'bytes':size,'verification':'visual_verified_from_uploaded_image','verification_note':'图片内容可可靠识别并已结构化入日期快照'})
with p.open('w',encoding='utf-8-sig',newline='') as f:
 w=csv.DictWriter(f,fieldnames=headers);w.writeheader();w.writerows(rows)
print('SOURCE_MANIFEST_MERGED added=12 total=%d'%len(rows))
