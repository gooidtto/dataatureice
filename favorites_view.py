...
def show_favorites_matrix(self):
    ...
    try:
        ...
        style.configure('Favorites.Treeview',font=FONT_BODY,rowheight=THEME['table_row_height'],background=THEME['table_bg'],fieldbackground=THEME['table_bg'],foreground=THEME['text'],borderwidth=0,relief='flat');style.configure('Favorites.Treeview.Heading',font=FONT_BODY,background=THEME['table_header'],foreground=THEME['text'],relief='flat',borderwidth=0,padding=(THEME['space_md'],3))
        ...
        for name in ('FavoritesPrimary.TButton','FavoritesDanger.TButton','FavoritesSecondary.TButton'):style.map(name,background=[('active',THEME['button_hover']),('pressed',THEME['button_pressed']),('disabled',THEME['button_disabled'])])
    ...
    for i,b in enumerate(blocks):
        ...
        if i:
            if model!=previous_model:
                ...
                line=tk.Frame(inner,height=THEME['period_gap'],background=THEME['surface'],highlightthickness=1,highlightbackground=THEME['border_soft']);line.pack(fill='x',pady=(THEME['space_xs'],THEME['space_xs']))
                ttk.Label(inner,text=f'历史时期 · {date}',style='FavoritesHint.TLabel',padding=(THEME['space_md'],THEME['space_xs'])).pack(fill='x')
        else:ttk.Label(inner,text=f'当前时期 · {date}',style='FavoritesHint.TLabel',padding=(THEME['space_md'],THEME['space_xs'])).pack(fill='x')
        ...
        tree.heading('favorite',text='收藏');tree.column('favorite',width=110,minwidth=70,anchor='center',stretch=False);iid=f'favorite-{i}';tree.insert('','end',iid=iid,values=[b.get(field,'') for field,_,_ in cols]+['★ 已收藏'],tags=('normal',));tree.tag_configure('normal',background=THEME['table_bg']);tree.tag_configure('selected',background=THEME['selection'])
        mapping[iid]={'_rows':list(b.get('_rows',[])),'_tree':tree,'_block':b,'_index':i};tree.bind('<Button-1>',lambda e,ii=iid,idx=i,t=tree:click(e,ii,idx,t),add='+');tree.bind('<B1-Motion>',lambda e,t=tree:drag_motion(e,t),add='+');tree.bind('<ButtonRelease-1>',drag_end,add='+');tree.bind('<Double-1>',lambda e,bb=b:self.detail_rows(bb.get('_rows',[])));tree.pack(fill='x',expand=True);trees.append(tree);previous_model=model;previous_date=date
