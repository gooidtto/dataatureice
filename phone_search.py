...
class App:
 ...
 def ui(self):
  top=ttk.Frame(self.root,padding=10);top.pack(fill='x');ttk.Label(top,text='🔎 品牌 / 系列 / 型号 / 别名',font=('微软雅黑',11,'bold')).pack(side='left');self.q=tk.StringVar();e=tk.Entry(top,textvariable=self.q,font=('微软雅黑',14),width=30);e.pack(side='left',padx=10,ipady=4);e.bind('<Return>',lambda _:self.search());self.cat=tk.StringVar(value='全部');ttk.Combobox(top,textvariable=self.cat,values=['全部','手机','平板','电脑','其它'],state='readonly',width=8).pack(side='left',padx=4);self.hist=tk.BooleanVar();ttk.Checkbutton(top,text='包含历史',variable=self.hist,command=self.search).pack(side='left',padx=8)
  ...
  self.status=ttk.Label(top,text='');self.status.pack(side='right');info=ttk.Frame(self.root,padding=(10,0,10,8));info.pack(fill='x');self.target=ttk.Label(info,text='搜索结果按图片原始价格条件逐行显示',font=('微软雅黑',11,'bold'));self.target.pack(side='left');self.meta=ttk.Label(info,text='',foreground='#666');self.meta.pack(side='right');act=ttk.Frame(self.root,padding=(10,0,10,8));act.pack(fill='x')
  ...
  ttk.Label(act,text='结果包含日期、条件、单位、备注、来源图片',foreground='#666').pack(side='right');f=ttk.Frame(self.root);f.pack(fill='both',expand=True,padx=10);self.tree=ttk.Treeview(f,columns=[x[0] for x in COLS],show='headings',selectmode='extended')
  ...
  y=ttk.Scrollbar(f,orient='vertical',command=self.tree.yview);x=ttk.Scrollbar(f,orient='horizontal',command=self.tree.xview);self.tree.configure(yscrollcommand=y.set,xscrollcommand=x.set);self.tree.grid(row=0,column=0,sticky='nsew');y.grid(row=0,column=1,sticky='ns');x.grid(row=1,column=0,sticky='ew');f.grid_rowconfigure(0,weight=1);f.grid_columnconfigure(0,weight=1);self.tree.bind('<Control-c>',self.copy);self.tree.bind('<Control-C>',self.copy);self.tree.bind('<Control-a>',lambda e:(self.tree.selection_set(self.tree.get_children()),'break')[1]);self.tree.bind('<Control-A>',lambda e:(self.tree.selection_set(self.tree.get_children()),'break')[1]);self.tree.bind('<Button-3>',self.menu);self.tree.bind('<Double-1>',self.detail)
 def load(self):
  self.s.load();self.meta.config(text=f'最新：{self.s.latest or "无"} · 快照 {len(self.s.dates)} · 已验证价格行 {len(self.s.rows)} · 图片 {sum(clean(r.get("include")).lower() in TRUE for r in self.s.manifest)}');self.search();self.status.config(text=('数据校验提示：'+self.s.errors[0] if self.s.errors else '数据校验通过'))
 def search(self):
 ...
 def detail(self,event=None):
  ...
  if not rows and event:
   ...
   if item:self.tree.selection_set(item);rows=self.selected()
  if not rows:return 'break' if event else None
  r=rows[0];w=tk.Toplevel(self.root);w.title(f'价格详情 · {r.get("model","")}');w.geometry('920x650');w.minsize(720,520);top=ttk.Frame(w,padding=15);top.pack(fill='x');ttk.Label(top,text=r.get('model',''),font=('微软雅黑',18,'bold')).pack(anchor='w');ttk.Label(top,text=f'{r.get("brand","")} / {r.get("series","")} · {r.get("data_date","")}',font=('微软雅黑',11)).pack(anchor='w',pady=(5,0));f=ttk.Frame(w,padding=15);f.pack(fill='both',expand=True);text=tk.Text(f,font=('微软雅黑',11),wrap='word');text.pack(fill='both',expand=True);labels=[('分类','category'),('子类型','subtype'),('型号代码','model_code'),('别名','alias'),('价格条件','condition'),('价格','price'),('单位','unit'),('备注','note'),('来源图片','source_image'),('原始层级','source_path'),('来源','origin'),('可信度','confidence'),('核验','verification')];text.insert('end','\n'.join(f'{h}：{r.get(k,"")}' for h,k in labels));text.configure(state='disabled');ttk.Button(w,text='关闭',command=w.destroy).pack(pady=10);return 'break' if event else None
 def sources(self):
  ...
  for r in self.s.manifest:
   if clean(r.get('include')).lower() in TRUE:t.insert('','end',values=(r.get('data_date',''),r.get('category',''),r.get('subtype',''),r.get('source_path','')))
  ttk.Label(w,text=f'纳入 {sum(clean(r.get("include")).lower() in TRUE for r in self.s.manifest)} 张；排除 {sum(clean(r.get("include")).lower() not in TRUE for r in self.s.manifest)} 张；source_path 是数据事实的原始层级引用').pack(anchor='w',padx=15,pady=5)
 def open_dir(self):
  try:
