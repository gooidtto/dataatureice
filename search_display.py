...
def _char_width(text):
    width = 0
    for ch in clean(text): width += 2 if unicodedata.east_asian_width(ch) in {"W", "F"} else 1
    return width
...
def build_display_columns(rows):
    """Create condition columns from the actual search result, keeping names verbatim."""
    condition_reps = {}
    for row in rows:
        condition = clean(row.get("condition", ""))
        if condition: condition_reps.setdefault(_condition_key(condition), row)
    ...
    for (_identity_key, date), group in grouped.items():
        ...
        for field, title, _width in columns[2:-1]:
            values_by_field[field].extend(clean(r.get("price", "")) for r in group if _condition_key(r.get("condition", "")) == _condition_key(title))
...
def _model_sort_key(periods):
    ...
    return tuple(-x for x in value_rank(ordered[0])) + (build_identity(ordered[0]),)


def normalize_search_results(rows):
    """Build model blocks; dates are newest first, one blank row between dates and two between models."""
    ...
    for model_index, (_identity_key, periods) in enumerate(model_groups):
        ...
        for period_index, (date, group) in enumerate(periods):
            if period_index > 0:
                result.append({"_separator": "period", "_model_index": model_index, "_period_index": period_index, "_columns": columns})
            condition_values = {}
            source_images = []
            for row in group:
            ...
            out = {
                ...
                "_model_key": tuple(clean(group[0].get(field, "")) for field in IDENTITY_FIELDS),
                "_period_key": date,
            ...
            for field, title, _width in columns[2:]:
                if field == "source_image": continue
                out[field] = " / ".join(condition_values.get(_condition_key(title), []))
            result.append(out)
        if model_index < len(model_groups) - 1:
            result.append({"_separator": "model", "_model_index": model_index, "_columns": columns})
    return result
