...
def sources(self):
    ...
    headings = {"date":"日期", "image":"来源图片", "path":"来源路径", "include":"纳入", "note":"备注"}; widths = {"date":110, "image":220, "path":420, "include":80, "note":220}
    for c in cols: tree.heading(c, text=headings[c]); tree.column(c, width=widths[c], anchor="w")
    y=ttk.Scrollbar(w,orient="vertical",command=tree.yview); x=ttk.Scrollbar(w,orient="horizontal",command=tree.xview); tree.configure(yscrollcommand=y.set,xscrollcommand=x.set); tree.grid(row=0,column=0,sticky="nsew"); y.grid(row=0,column=1,sticky="ns"); x.grid(row=1,column=0,sticky="ew"); w.grid_rowconfigure(0,weight=1); w.grid_columnconfigure(0,weight=1)
...
def _remove_favorite_rows(self,window,rows):
    if not rows:return messagebox.showinfo("我的收藏","请先选择要移除的收藏",parent=window)
...
def _favorite_popup_menu(self,event,tree,mapping,window):
    ...
    if not row:return
    tree.selection_set(iid); menu=tk.Menu(tree,tearoff=False); menu.add_command(label="查看详情",command=lambda:self.detail_rows([row])); menu.add_command(label="移除收藏",command=lambda:self._remove_favorite_rows(window,[row])); menu.add_command(label="复制",command=lambda:self.copy_popup([row])); menu.tk_popup(event.x_root,event.y_root)
...
def stats(self):
    ...
    by_date=Counter(r.get("data_date","") for r in rows); by_cond=Counter(r.get("condition","") for r in rows); text=[f"价格记录：{len(rows)} 条",f"型号目标：{len({__import__('phone_search').rid(r) for r in rows})} 个","","按日期："]; text += [f"  {d}: {n} 条" for d,n in sorted(by_date.items(),reverse=True)]; text += ["","按价格条件："]+[f"  {c}: {n} 条" for c,n in by_cond.most_common()]; messagebox.showinfo("条件统计","\n".join(text),parent=self.root)
...
def show_compare(self,rows,targets=None):
    ...
    if not rows:return
    history=self.s.history(targets); w=_new_window(self,"历史价格对比 · 同型号跨日期","1650x760",(1100,600)); cols=("date","model","condition","price","unit"); frame=ttk.Frame(w,padding=12); frame.pack(fill="both",expand=True); tree=ttk.Treeview(frame,columns=cols,show="headings",selectmode="extended")
...
def detail_rows(self,rs):
    rows=list(rs or [])
    if not rows:return
    row=rows[0]; w=_new_window(self,"记录详情","760x560",(600,420)); text=tk.Text(w,wrap="none",font=("微软雅黑",10)); text.pack(fill="both",expand=True,padx=12,pady=12)
