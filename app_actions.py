...
def _reliable_search(self, record_history=True):
    ...
    try:
        ...
        self.status.config(text=f"搜索失败：{exc}")
        return []
...
def sources(self):
    ...
    y = ttk.Scrollbar(w, orient="vertical", command=tree.yview); x = ttk.Scrollbar(w, orient="horizontal", command=tree.xview)
    tree.configure(yscrollcommand=y.set, xscrollcommand=x.set); tree.grid(row=0,column=0,sticky="nsew"); y.grid(row=0,column=1,sticky="ns"); x.grid(row=1,column=0,sticky="ew"); w.grid_rowconfigure(0,weight=1); w.grid_columnconfigure(0,weight=1)
...
def show_favorites(self):
    rows = self.fav.dedupe(); w = _new_window(self, "⭐ 我的收藏", "1500x760", (1050, 560))
...
def stats(self):
    ...
    text=[f"价格记录：{len(rows)} 条", f"型号目标：{len({__import__('phone_search').rid(r) for r in rows})} 个", "", "按日期："]; text += [f"  {d}: {n} 条" for d,n in sorted(by_date.items(),reverse=True)]; text += ["", "按价格条件："] + [f"  {c}: {n} 条" for c,n in by_cond.most_common()]; messagebox.showinfo("条件统计","\n".join(text),parent=self.root)
