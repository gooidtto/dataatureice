...
def discover(data_dir):
    ...
    for n in sorted(os.listdir(data_dir)):
        ...
        if DATE_FILE.fullmatch(n) and os.path.isfile(p):
            out.append(('root',n[:10],p))
...
def normalize_rows(rows,date,path,errors):
    ...
    for line,r0 in enumerate(rows,2):
        ...
        if r['data_date']!=date:
            errors.append(f'{path}:{line}: data_date={r["data_date"]!r} 与快照 {date} 不一致')
        if r['category'] not in CATS:
            errors.append(f'{path}:{line}: 非标准分类 {r["category"]!r}')
        for f in required:
            if not r[f]:
                errors.append(f'{path}:{line}: {f}为空')
        if r['verified'].lower() not in TRUE:
            errors.append(f'{path}:{line}: verified={r["verified"]!r}')
        result.append(r)
    return result
...
def validate_unique(rows,date,label,errors):
    ...
    if dups:
        errors.append(f'{date}: {label} record_id重复 {sum(v-1 for v in dups.values())} 行 / {len(dups)} 个ID')
...
def validate(data_dir):
    ...
    if not files:
        errors.append('未找到价格快照')
        return errors,warnings,[]

    by_source_date={}
    for source,date,path in files:
        by_source_date.setdefault((source,date),[]).append(path)
    ...
    for (source,date),paths in sorted(by_source_date.items()):
        ...
        for path in paths:
            try:
                ...
                errors.append(f'{path}: CSV读取失败: {e}')
                continue
            if not rows:
                warnings.append(f'{path}: 空快照分片')
                continue
    ...
    # The canonical database is the runtime source. Snapshots are the restore/reference source.
    # When both exist for a date, compare the complete 19-field record set instead of counting
    # the same facts twice as if they were duplicate records.
    dates=sorted(set(date for _,date,_ in files))
    all_rows=[]
    for date in dates:
        db=source_rows.get(('database',date),[])
        snapshots=source_rows.get(('snapshots',date),[])
        root=source_rows.get(('root',date),[])
        canonical=db or root or snapshots
        if date in EXPECTED_COUNTS and len(canonical)!=EXPECTED_COUNTS[date]:
            errors.append(f'{date}: 行数错误，期望 {EXPECTED_COUNTS[date]}，实际 {len(canonical)}')
        if db and snapshots:
            a=sorted(row_tuple(r) for r in db)
            b=sorted(row_tuple(r) for r in snapshots)
            if a!=b:
                errors.append(f'{date}: database 与 snapshots 的19字段记录集不一致')
        all_rows.extend(canonical)

    mp=os.path.join(data_dir,'source_image_manifest.csv')
    if not os.path.isfile(mp):
        errors.append('缺失 source_image_manifest.csv')
    else:
        try:
            manifest=read_csv(mp)
        except Exception as e:
            errors.append(f'来源清单读取失败: {e}')
            manifest=[]
        ...
        for item in sorted(data_sources-included):
            errors.append(f'数据来源未在清单纳入: {item[0]} / {item[1]}')
        for item in sorted(included-data_sources):
            warnings.append(f'清单纳入但当前无价格行: {item[0]} / {item[1]}')
