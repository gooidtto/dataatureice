...
def visible_matrix_rows(rows):
    """Adapt the authoritative search order for display without re-sorting it."""
    return [row for row in normalize_search_results(list(rows or [])) if not row.get("_separator")]
...
def raw_rows_for_payloads(payloads):
    """Flatten payload rows while preserving their search/display order."""
    out = []
    seen = set()
...
def selected_payloads(app):
    """Return selected payloads in their displayed order, never selection-widget order."""
    selected_ids = set(app.tree.selection())
    matrix_map = getattr(app, "_matrix_map", {})
    payloads = [payload for payload in matrix_map.values() if payload and not payload.get("_separator") and payload.get("_tree_iid") in selected_ids]
    return sorted(payloads, key=lambda p: p.get("_display_index", 0))
...
def add_favorite(app):
    """One-click favorite records the complete current search, across all dates."""
...
def copy_selected(app, event=None):
    ...
    if not payloads:
        app.toast("搜索区域只读；请到“我的收藏”选择后复制")
...
def _export_csv(app, payloads, path):
    ...
    with open(path, "w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(matrix_headers(raw))
        last_model = last_date = None
        for payload in payloads:
            model = payload.get("_model_key")
            date = payload.get("_period_key")
            if last_model is not None and model != last_model:
                ...
                writer.writerow([""] * len(columns))
            writer.writerow(matrix_values(payload, columns))
            last_model, last_date = model, date
...
def _export_xlsx(app, payloads, path):
    raw = [r for p in payloads for r in p.get("_rows", [])]
    columns = _columns(raw)
    headers = matrix_headers(raw)
    wb = Workbook()
    ws = wb.active
    ws.title = "搜索结果"
    for col, label in enumerate(headers, 1):
        ws.cell(1, col, label).font = Font(bold=True)
    row_index = 2
    last_model = last_date = None
    for payload in payloads:
        model = payload.get("_model_key")
        ...
        if last_model is not None and model != last_model:
            ...
            row_index += 1
        for col, value in enumerate(matrix_values(payload, columns), 1):
            ws.cell(row_index, col, value)
        row_index += 1
        last_model, last_date = model, date
    ws.freeze_panes = "A2"
    for col, (_field, title, width) in enumerate(columns, 1):
        ws.column_dimensions[ws.cell(1, col).column_letter].width = max(10, min(60, width / 8))
    wb.save(path)
...
def export_csv(app):
    ...
    app.status.config(text=f"已导出搜索结果 {len(payloads)} 个结果块")
