...
def _condition_key(value):
    return phone_search.key(value)


def matrix_values(display_row, columns):
    """Map block-local raw rows onto the single global display-column schema."""
    rows = list(display_row.get("_rows", []) or [])
    first = rows[0] if rows else {}
    values = []
    for field, label, _width in columns:
        if field.startswith("condition_"):
            wanted = _condition_key(label)
            prices = [
                str(r.get("price", ""))
                for r in rows
                if _condition_key(r.get("condition", "")) == wanted and str(r.get("price", "")) != ""
            ]
            values.append(" / ".join(prices))
        elif field == "identity":
            values.append(display_row.get("identity", ""))
        elif field == "source_image":
            values.append(display_row.get("source_image", first.get("source_image", "")))
        elif field == "data_date":
            values.append(first.get("data_date", ""))
        else:
            values.append(display_row.get(field, first.get(field, "")))
    return values
...
def copy_all(app):
    ...
    app.root.update()
    app.status.config(text=f"已复制搜索结果 {len(app.rows)} 条，保留分组与间隔")
...
def _export_xlsx(app, payloads, path, title="搜索结果"):
    ...
    for payload in payloads:
        ...
        last_model, last_date = model, date
    ws.freeze_panes = "A2"
    for col, (_field, _title, width) in enumerate(columns, 1):
...
def export_csv(app):
    ...
    app.status.config(text=f"已导出搜索结果 {len(app.rows)} 条，保留分组与间隔")
