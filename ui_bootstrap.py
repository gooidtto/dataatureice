...
def favorite_groups(self):
    ...
    for r in self.fav.dedupe():
        model_key=phone_search.key(r.get("model", ""))
        groups.setdefault(model_key, []).append(r)
    ordered=[]
    for model_key, rows in groups.items():
        rows=sorted(rows, key=lambda r:(r.get("data_date", ""), phone_search.num(r.get("price", "")) if phone_search.num(r.get("price", "")) is not None else float("-inf")), reverse=True)
        newest=rows[0].get("data_date", "") if rows else ""
        blocks=[]
        for date in sorted({r.get("data_date", "") for r in rows}, reverse=True):
            blocks.append([r for r in rows if r.get("data_date", "") == date])
        ordered.append((model_key, newest, blocks))
    ordered.sort(key=lambda x:(x[1], x[0]), reverse=True)
    return [(model_key, blocks) for model_key, _, blocks in ordered]
...
def _standardize_window(w):
    try:
        ...
        x=max(0,px+(pw-width)//2); y=max(0,py+(ph-height)//2); w.minsize(min_w,min_h); w.geometry(f"{width}x{height}+{x}+{y}")
