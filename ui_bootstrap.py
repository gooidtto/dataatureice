...
class SearchApp(phone_search.App):
    ...
    def ui(self):
        ...
        # Reference-style search surface: white, thin border, black text, blue focus.
        ...
        self.cat = ttk.Combobox(root, textvariable=tk.StringVar(value="全部"), values=["全部", "手机", "平板", "电脑", "其它", "手机配件"], state="readonly", width=9)
        self.cat.set("全部")
        self.cat.place(relx=1.0, x=-THEME["space_lg"], y=THEME["space_lg"] + 4, anchor="ne")
        ...
        # The search-result section is now the single common action surface.
    ...
    def _history_popup(self):
        ...
        try:
            ...
            popup.bind("<Escape>", lambda _e: self._hide_suggestions_ui())
    ...
    def _refresh_suggestions_ui(self):
        """Always render local persistent history; never silently hide it on no-match."""
        if not hasattr(self, "entry") or not hasattr(self, "h"): return
        try:
            query = phone_search.clean(self.q.get())
            matches = list(self.h.suggestions(query, 12))
            items = matches
            mode = "匹配历史" if query and matches else "最近搜索"
            if query and not matches:
                items = list(self.h.suggestions("", 12))
            popup = self._history_popup()
            if popup is None: return
            for child in popup.winfo_children(): child.destroy()
            outer = tk.Frame(popup, bg=THEME["surface"], bd=1, relief="solid", highlightthickness=1, highlightbackground=THEME["border"])
            outer.pack(fill="both", expand=True)
            header = tk.Frame(outer, bg=THEME["surface"], height=44)
            ...
            body = tk.Frame(outer, bg=THEME["surface"], bd=0)
            body.pack(fill="both", expand=True)
            if items:
                for item in items:
                    row = tk.Frame(body, bg=THEME["surface"], height=40)
                    row.pack(fill="x"); row.pack_propagate(False)
                    marker = tk.Label(row, text="◷", bg=THEME["surface"], fg=THEME["text_muted"], font=("SimHei", 12), width=3)
                    marker.pack(side="left")
                    label = tk.Label(row, text=item, anchor="w", bg=THEME["surface"], fg=THEME["text"], font=FONT_BODY, padx=THEME["space_xs"], cursor="hand2")
                    label.pack(side="left", fill="both", expand=True)
                    for widget in (row, marker, label):
                        widget.bind("<Button-1>", lambda _e, value=item: self._use_suggestion(value))
                        widget.bind("<Enter>", lambda _e, w=row: self._history_hover(w, True))
                        widget.bind("<Leave>", lambda _e, w=row: self._history_hover(w, False))
            else:
            ...
            popup.update_idletasks()
            width = max(self.entry.winfo_width(), 460)
            height = max(120, min(44 + len(items) * 40 + 2, 520))
            x = self.entry.winfo_rootx()
            y = self.entry.winfo_rooty() + self.entry.winfo_height() + 4
    ...
    def _history_hover(self, row, active):
        try:
            bg = THEME["selection"] if active else THEME["surface"]
            row.configure(bg=bg)
    ...
    def clear_search_history(self):
        try: self.h.clear()
            ...
            self.status.config(text=f"清空搜索历史失败：{exc}"); return
        self.status.config(text="搜索历史已清空")
        self.entry.focus_set(); self._refresh_suggestions_ui()

    def _use_suggestion(self, value):
        self._hide_suggestions_ui(); self.q.set(value); self.search()
    ...
    def _dismiss_suggestions(self, event=None):
        ...
        try:
            ...
            if px <= ex <= px + self.entry.winfo_width() and py <= ey <= py + self.entry.winfo_height():
                self.root.after_idle(self._refresh_suggestions_ui); return
...
def _configure_result_tree(tree, columns, iid, display, favorite):
    ...
    for field, title, width in columns:
        anchor = "center" if field == "data_date" or field.startswith("condition_") else "w"
        tree.heading(field, text=title)
        tree.column(field, width=width, minwidth=max(60, min(width, 90)), anchor=anchor, stretch=True)
    ...
    values.append("★ 已收藏" if favorite else "☆ 收藏")
    tag = f"m{display['_model_index']}d{display['_period_index']}"
...
def _render_search_matrix(self, result):
    _clear_result_views(self); self.map = {}; self._matrix_map = {}; self._display_columns = ()
    blocks = build_result_blocks(list(result or [])); parent = getattr(self, "_results_inner", None)
    ...
    for block in blocks:
        ...
        if previous_model is not None and model_index != previous_model:
            for _ in range(2):
                spacer = tk.Frame(parent, height=THEME["model_gap"], background=THEME["model_separator"], highlightthickness=1, highlightbackground=THEME["model_separator_line"])
                spacer.pack(fill="x", pady=(THEME["space_xs"], THEME["space_xs"])); self._result_views.append(spacer)
        elif period_index > 0:
            spacer = tk.Frame(parent, height=THEME["period_gap"], background=THEME["period_separator"], highlightthickness=1, highlightbackground=THEME["separator_line"])
            spacer.pack(fill="x", pady=(THEME["space_xs"], THEME["space_xs"])); self._result_views.append(spacer)
        columns = tuple(block.get("_columns") or ()); self._display_columns = columns or self._display_columns
        iid = f"result-{len(self._matrix_map)}"; rows = list(block.get("_rows") or [])
        favorite_keys = {self.fav.identity(r) for r in self.fav.dedupe()}; favorite = any(self.fav.identity(r) in favorite_keys for r in rows)
        ...
        tree.bind("<Double-1>", lambda _event, value=iid: self._result_double_click(value))
        def on_click(event, value=iid):
            if tree.identify_column(event.x) == f"#{len(columns) + 1}":
                self._result_action_favorite(value); return "break"
        tree.bind("<Button-1>", on_click, add="+")
        tree.pack(fill="x", expand=True); frame.pack(fill="x", expand=True, pady=(0, THEME["result_gap"]))
    ...
    try:
        ...
        self.result_count.config(text=f"{len(self.rows)} 条 · {len(self._matrix_map)} 结果块")
    except tk.TclError: pass
...
def _apply_async_result(self, query_id, q, record_history, future):
    ...
    try: result = future.result()
        ...
        self.status.config(text=f"搜索失败：{type(exc).__name__}: {exc}"); return
