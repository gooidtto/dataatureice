...
def _new_window(self, title, geometry=None, minsize=None):
    ...
    w.geometry(f"{width}x{height}+{x}+{y}"); w.minsize(min(width, x + width), min(height, y + height))
...
class SearchApp(phone_search.App):
    ...
    def ui(self):
        ...
        try:
            ...
            button_base = dict(background=THEME["button_bg"], font=FONT_BODY,
                               padding=(THEME["button_pad_x"], THEME["button_pad_y"]), relief="solid", borderwidth=1)
            style.configure("TButton", **button_base, foreground=THEME["button_text"])
            ...
            for name in ("Primary.TButton", "Favorite.TButton", "Compare.TButton"):
                style.configure(name, **button_base, foreground=THEME["button_accent_text"])
        ...
        top.pack(fill="x", padx=THEME["space_lg"], pady=(THEME["space_lg"], THEME["space_sm"])); self.search_bar = top
        ...
        self.entry.bind("<Return>", lambda _e: self.search()); self.entry.bind("<FocusIn>", lambda _e: self.root.after_idle(self._refresh_suggestions))
        self.cat = ttk.Combobox(top, textvariable=tk.StringVar(value="全部"), values=["全部", "手机", "平板", "电脑", "其它", "手机配件"], state="readonly", width=8)
        self.cat.set("全部"); self.cat.pack(side="left", padx=THEME["space_xs"])
        for text, command, style_name in (("×", self.clear_search, "TButton"), ("查询", self.search, "Primary.TButton"), ("刷新", self.load, "TButton"), ("数据目录", self.open_dir, "TButton"), ("来源结构", self.sources, "TButton")):
        ...
        info = ttk.Frame(root, style="Info.TFrame", padding=(THEME["space_lg"], THEME["space_sm"], THEME["space_lg"], THEME["space_md"])); info.pack(fill="x")
        self.target = ttk.Label(info, text="输入品牌、系列、型号开始查询", style="Title.TLabel"); self.target.pack(side="left")
        self.meta = ttk.Label(info, text="", style="Meta.TLabel"); self.meta.pack(side="right")
        actions = ttk.Frame(root, style="Action.TFrame", padding=(THEME["space_lg"], 0, THEME["space_lg"], THEME["space_md"])); actions.pack(fill="x")
        ...
        host = ttk.Frame(root, style="Results.TFrame", padding=1); host.pack(fill="both", expand=True, padx=THEME["space_lg"], pady=(0, THEME["space_lg"]))
        self._results_canvas = tk.Canvas(host, highlightthickness=0, bd=0, background=THEME["surface"], relief="flat")
        scrollbar = ttk.Scrollbar(host, orient="vertical", command=self._results_canvas.yview); self._results_canvas.configure(yscrollcommand=scrollbar.set)
        self._results_canvas.grid(row=0, column=0, sticky="nsew"); scrollbar.grid(row=0, column=1, sticky="ns"); host.grid_rowconfigure(0, weight=1); host.grid_columnconfigure(0, weight=1)
        ...
        self._results_inner.bind("<Configure>", lambda _e: self._results_canvas.configure(scrollregion=self._results_canvas.bbox("all"))); self._results_canvas.bind("<Configure>", self._resize_results_inner)
        self.empty_hint = ttk.Label(host, text="输入品牌、系列、型号开始查询", font=FONT_TITLE, foreground=THEME["text_muted"], background=THEME["surface"])
        self._result_views=[]; self._result_trees=[]; self._result_tree_map={}; self._result_order=[]; self._matrix_map={}; self._display_columns=()
        self.tree = ttk.Treeview(host, columns=[x[0] for x in phone_search.COLS] + ["favorite"], show="headings", selectmode="none"); self.tree.grid_remove()
    ...
    def _refresh_suggestions(self):
        items=self.h.suggestions(phone_search.clean(self.q.get()),5); panel=getattr(self,"_history_panel",None)
        ...
        try:
            ...
            tk.Label(panel,text="搜索历史",anchor="w",bg=THEME["surface_alt"],fg=THEME["text_secondary"],font=FONT_LABEL,padx=THEME["space_md"],pady=THEME["space_sm"]).pack(fill="x")
            for item in items:
                label=tk.Label(panel,text=item,anchor="w",bg=THEME["surface"],fg=THEME["text"],font=FONT_BODY,padx=THEME["space_md"],pady=THEME["space_sm"],cursor="hand2"); label.pack(fill="x")
                label.bind("<Button-1>",lambda _e,value=item:self._use_suggestion(value)); label.bind("<Enter>",lambda _e,widget=label:widget.configure(bg=THEME["selection"])); label.bind("<Leave>",lambda _e,widget=label:widget.configure(bg=THEME["surface"]))
...
def _configure_result_tree(tree,columns,iid,display,favorite):
    ...
    for field,title,width in columns:
        tree.heading(field,text=title); is_quote=field.startswith("condition_"); tree.column(field,width=width,minwidth=max(60,min(width,90)),anchor="center" if field=="data_date" or is_quote else "w",stretch=True)
    tree.heading("favorite",text="收藏"); tree.column("favorite",width=110,minwidth=90,anchor="center",stretch=True)
    values=[display.get(field,"") for field,_,_ in columns]; tag=_row_tag(display["_model_index"],display["_period_index"])
    tree.insert("","end",iid=iid,values=values+["★ 已收藏" if favorite else "☆ 一键收藏"],tags=(tag,)); tree.tag_configure(tag,background=THEME["model_bands"][display["_model_index"]%len(THEME["model_bands"])])
...
def _render_search_matrix(self,result):
    ...
    for block in blocks:
        ...
        if previous_model is not None and model_index!=previous_model:
            for _ in range(2):
                spacer=tk.Frame(parent,height=THEME["model_gap"],background=THEME["surface"],highlightthickness=1,highlightbackground=THEME["border_soft"]);spacer.pack(fill="x",pady=(THEME["space_xs"],THEME["space_xs"]));self._result_views.append(spacer)
        elif period_index>0:
        ...
        if columns:self._display_columns=columns
        iid=f"result-{len(self._matrix_map)}"; rows=list(block.get("_rows") or []); favorite_keys={self.fav.identity(r) for r in self.fav.dedupe()}; favorite=any(self.fav.identity(r) in favorite_keys for r in rows)
        frame=tk.Frame(parent,bd=0,highlightthickness=0,background=THEME["surface"]); tree=ttk.Treeview(frame,columns=(),show="headings",height=1,selectmode="none",style="Search.Treeview")
        _configure_result_tree(tree,columns,iid,block,favorite); tree.bind("<Double-1>",lambda _event,iid=iid:self._result_double_click(iid)); tree.pack(fill="x",expand=True); frame.pack(fill="x",expand=True,pady=(0,THEME["result_gap"]))
        self._result_views.append(frame); self._result_trees.append(tree); self._result_tree_map[iid]=tree; self._result_order.append(iid); self._matrix_map[iid]=block
        first=rows[0] if rows else {}; hidden_values=[first.get(c,"") for c in phone_search.COLS]+["★ 已收藏" if favorite else "☆ 一键收藏"]
        try:self.tree.insert("","end",iid=iid,values=hidden_values)
        except tk.TclError:pass
        if rows:
            self.map[iid]=rows[0]
            for row in rows:
    ...
    try:
        self._results_canvas.configure(scrollregion=self._results_canvas.bbox("all")); self.empty_hint.place_forget() if self._matrix_map else self.empty_hint.place(relx=0.5,rely=0.5,anchor="center")
    except tk.TclError:pass
...
def _apply_async_result(self,query_id,q,record_history,future):
    ...
    self.render(self.rows); self.target.config(text=f"搜索结果：{q} · {len(self._matrix_map)} 个结果块"); self.status.config(text=f"找到 {len(self._matrix_map)} 个结果块"); self._refresh_suggestions()
...
def clear_search(self):
    ...
    self._search_query_id=getattr(self,"_search_query_id",0)+1; self.q.set(""); self.hide_suggestions(); self.rows=[]; self.map={}; self._matrix_map={}; self._display_columns=()
    try:self.tree.delete(*self.tree.get_children())
    except tk.TclError:pass
    _clear_result_views(self); self.target.config(text="输入品牌、系列、型号开始查询")
    try:self.empty_hint.place(relx=0.5,rely=0.5,anchor="center")
    except tk.TclError:pass
