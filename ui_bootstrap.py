...
def clear_search(self):
    if getattr(self,'_auto_search_job',None):
        try:self.root.after_cancel(self._auto_search_job)
        except tk.TclError:pass
        self._auto_search_job=None
...
def _schedule_auto_search(self, *_event):
    if getattr(self,'_auto_search_job',None):
        try:self.root.after_cancel(self._auto_search_job)
        except tk.TclError:pass
        self._auto_search_job=None
    ...
    if not q:return
    self._auto_search_job=self.root.after(_AUTO_SEARCH_DELAY_MS, lambda:_run_auto_search(self, q))


def _run_auto_search(self, scheduled_q):
    self._auto_search_job=None
    current=phone_search.clean(self.q.get())
    if not current or current != scheduled_q:return
    self.search(True)
...
def ui(self):
    ...
    self._auto_search_job=None
    self.entry.bind('<KeyRelease>',lambda e:_schedule_auto_search(self,e),add='+')
...
def _select_result(self,iid,payload,event=None):
    ...
    if payload:self.status.config(text=f'已选择：{payload.get("identity","")} · {payload.get("data_date","")}')
...
def _render_search_matrix(self,result):
    ...
    for block_index,display in enumerate(display_rows):
        iid=str(block_index);raw_rows=display.get('_rows',[])
        self.tree.insert('','end',iid=iid,values=[display.get('data_date',''),display.get('identity','')])
        self.map[iid]=raw_rows[0] if raw_rows else None;self._matrix_map[iid]=display
        ...
        for col,(field,label,width) in enumerate(columns):
            ...
            h=tk.Label(block,text=label,font=('微软雅黑',10,'bold'),bg='white',anchor='center',justify='center',wraplength=max(width-12,80),padx=4,pady=3);h.grid(row=0,column=col,sticky='ew')
            v=tk.Label(block,text=display.get(field,''),font=('微软雅黑',10),bg='white',anchor='center' if field!='identity' else 'w',justify='center' if field!='identity' else 'left',wraplength=max(width-12,80),padx=4,pady=6);v.grid(row=1,column=col,sticky='ew')
            header.append(h);values.append(v)
        widgets=header+values+[block];self._matrix_widgets[iid]=widgets
        for widget in widgets:
    ...
    self._matrix_canvas.update_idletasks();self._matrix_canvas.configure(scrollregion=self._matrix_canvas.bbox('all'))
...
def search(self,record_history=True):
    ...
    if not q:
        ...
        for child in self._matrix_inner.winfo_children():child.destroy()
        self.target.config(text='输入品牌 / 系列 / 型号开始查询');self.empty_hint.place(relx=0.5,rely=0.5,anchor='center');self.status.config(text='请输入品牌、系列、型号或别名');return []
    if record_history:self.h.add(q)
    result=self.s.search(q,self.cat.get());display_rows=_render_search_matrix(self,result)
    self.target.config(text=f'搜索结果：{q} · {len(display_rows)} 个独立结果');self.status.config(text=f'找到 {len(display_rows)} 个独立结果');self.refresh_suggestions();return result
...
def _window_spec(title):
    ...
    specs=(("搜索历史",560,620,420,420),("我的收藏",1500,760,1050,560),("记录详情",1500,620,1000,480),("历史价格对比",1650,760,1100,600),("来源图片结构",1100,620,800,480))
    for marker,sw,sh,smw,smh in specs:
        if marker in title:return sw,sh,smw,smh
...
def _standardize_window(w):
    ...
    try:
        ...
        title=phone_search.clean(w.title());screen_w=w.winfo_screenwidth();screen_h=w.winfo_screenheight();pref_w,pref_h,min_w,min_h=_window_spec(title);width,height,min_w,min_h=_fit_geometry(screen_w,screen_h,pref_w,pref_h,min_w,min_h,0.94,0.90);w.minsize(min_w,min_h);_center_on_screen(w,width,height);w.transient(w.winfo_toplevel());w.bind('<Escape>',lambda _e:w.destroy(),add='+');w.protocol('WM_DELETE_WINDOW',w.destroy);w.lift();w.focus_force()
    except tk.TclError:pass
...
def main():
    root=tk.Tk();phone_search.App(root);_standardize_main_window(root);root.after_idle(lambda:root.focus_force());root.mainloop()
