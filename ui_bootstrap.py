...
def _apply_async_result(self,query_id,q,category,record_history,future):
    if query_id!=getattr(self,'_search_query_id',0) or q!=phone_search.clean(self.q.get()):return
    try:result=future.result()
    except Exception as exc:
        self.status.config(text=f'搜索失败：{exc}'); return
    if query_id!=getattr(self,'_search_query_id',0) or q!=phone_search.clean(self.q.get()):return
...
def _debounced_search(self):
    ...
    if not q:return
    self._search_query_id=getattr(self,'_search_query_id',0)+1; query_id=self._search_query_id; category=self.cat.get(); future=_SEARCH_EXECUTOR.submit(self.s.search,q,category); future.add_done_callback(lambda f:self.root.after(0,lambda:self._apply_async_result(query_id,q,category,False,f)))
...
def search(self,record_history=True):
    ...
    if getattr(self,'_search_after_id',None):
        ...
        self._search_after_id=None
    self._search_query_id=getattr(self,'_search_query_id',0)+1; query_id=self._search_query_id; category=self.cat.get(); future=_SEARCH_EXECUTOR.submit(self.s.search,q,category); self.status.config(text='正在搜索…'); future.add_done_callback(lambda f:self.root.after(0,lambda:self._apply_async_result(query_id,q,category,record_history,f))); return None
