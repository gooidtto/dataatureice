...
def test_favorite_groups_keep_same_model_together_and_dates_descending():
    spec2 = importlib.util.spec_from_file_location("ui_bootstrap", ROOT / "ui_bootstrap.py")
    ui = importlib.util.module_from_spec(spec2)
    spec2.loader.exec_module(ui)
    ...
    assert [key[3] for key, _blocks in groups] == ["M1", "M2"]
    assert all(len(blocks) == 2 for _key, blocks in groups)
    assert [blocks[0][0]["data_date"] for _key, blocks in groups] == ["2026-08-31", "2026-08-31"]
    assert [blocks[1][0]["data_date"] for _key, blocks in groups] == ["2026-08-20", "2026-08-20"]
    # Historical price must not determine model ordering.
    assert [key[3] for key, _blocks in groups] == ["M1", "M2"]
