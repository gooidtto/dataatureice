...
def test_display_block_contract_keeps_dynamic_columns_local():
    ui = _ui_module()
    blocks = ui.build_result_blocks([row(model="M1", condition="开机屏好", price="100", source_image="a.jpg"), row(model="M2", condition="不开机", price="80", source_image="b.jpg")])
    assert len(blocks) == 2
...
def test_normalize_is_only_a_separator_adapter_over_canonical_blocks():
    ui = _ui_module()
    rows = [row(model="M1", data_date="2026-08-31", condition="好"), row(model="M1", data_date="2026-08-20", condition="坏"), row(model="M2", data_date="2026-08-31", condition="不开机")]
    blocks = ui.build_result_blocks(rows)
    display = [item for item in ui.normalize_search_results(rows) if not item.get("_separator")]
    assert display == blocks
