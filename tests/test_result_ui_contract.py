...
def _load_module(path): return ast.parse(path.read_text(encoding="utf-8"))
def _function(module,name): return next(node for node in module.body if isinstance(node,ast.FunctionDef) and node.name==name)

def test_window_helper_signatures_are_ast_stable():
    module=_load_module(ROOT/"app_actions.py");assert [arg.arg for arg in _function(module,"show_compare").args.args]==["self","rows","targets"];assert [arg.arg for arg in _function(module,"detail_rows").args.args]==["self","rs"]
