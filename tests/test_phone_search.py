...
def _row(rid,date,model,price,condition='开机屏好',model_code=''):
    r={k:'' for k in mod.FIELDS};r.update(record_id=rid,data_date=date,category='手机',subtype='',brand='Realme',series='X系列',model=model,model_code=model_code,condition=condition,price=str(price),unit='元',verified='1');return r
def test_search_is_all_dates_newest_first_then_price(tmp_path):
    ...
    for date,rows in {'2026-08-31':[_row('1','2026-08-31','Realme X7',160),_row('2','2026-08-31','Realme X7 Pro',170),_row('3','2026-08-31','Realme X7',170)],'2026-08-25':[_row('4','2026-08-25','Realme X7',150),_row('5','2026-08-25','Realme X7',140)]}.items():
...
def test_history_matches_same_model_across_dates():
    ...
    got=s.history([s.rows[0]]);assert [(x['data_date'],x['model']) for x in got]==[('2026-08-31','Realme X7'),('2026-08-25','Realme X7')]
def test_history_keeps_network_model_identity_separate():
    s=mod.Store('unused');s.rows=[_row('1','2026-08-31','Realme X7',170,model_code='RMX3085'),_row('2','2026-08-31','Realme X7',150,model_code='RMX3086')]
    got=s.history([s.rows[0]])
    assert [x['model_code'] for x in got]==['RMX3085']
def test_search_history_deduplicates(tmp_path):
    h=mod.JsonList(str(tmp_path/'search_history.json'));h.add('Realme X7');h.add('iPhone 15');h.add('realme-x7');assert len(h.items)==2;assert mod.key(h.items[0])==mod.key('Realme X7')
