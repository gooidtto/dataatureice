...
def test_different_model_blocks_do_not_share_dynamic_columns():
    rows=[row(record_id="a",model="畅玩8x (3+32)",model_code="BNK-AL00",condition="开机好碎",price="900"),row(record_id="b",model="畅玩7x (3+32)",model_code="BND-AL00",condition="开机碎屏",price="700")];blocks=payloads(normalize_search_results(rows));assert set(labels(blocks[0]["_columns"])[2:-1])=={"开机好碎"};assert set(labels(blocks[1]["_columns"])[2:-1])=={"开机碎屏"}
