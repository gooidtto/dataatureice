...
def test_each_result_block_owns_only_its_own_condition_columns():
    ...
    assert all("废板·整机" not in labels(blocks[0]["_columns"]) for _ in [0]); assert all("开机靓好" not in labels(blocks[1]["_columns"]) for _ in [0])
...
def test_different_model_blocks_do_not_share_dynamic_columns():
    ...
    blocks=payloads(normalize_search_results(rows)); assert set(labels(blocks[0]["_columns"])[2:-1])=={"开机好碎"}; assert set(labels(blocks[1]["_columns"])[2:-1])=={"开机碎屏"}
...
def test_duplicate_condition_rows_are_not_silently_dropped():
    ...
    display=payloads(normalize_search_results(rows))[0]; field=next(field for field,title,_width in build_display_columns(rows) if title=="开机靓好"); assert display[field]=="700 / 680"; assert len(display["_rows"])==2
...
def test_different_periods_are_newest_first_with_one_blank_separator():
    ...
    result=normalize_search_results(rows); separators=[r for r in result if r.get("_separator")]; assert [r["data_date"] for r in payloads(result)]==["2026-08-31","2026-08-25"]; assert len([r for r in separators if r.get("_separator")=="period"])==1; assert [r["_period_index"] for r in payloads(result)]==[0,1]
...
def test_different_models_get_two_blank_separators():
    ...
    result=normalize_search_results(rows); assert sum(1 for r in result if r.get("_separator")=="model")==2; assert [r["_model_index"] for r in payloads(result)]==[0,1]
...
def test_same_model_different_periods_have_distinct_period_indexes():
    ...
    result=payloads(normalize_search_results(rows)); assert [(r["data_date"],r["_model_index"],r["_period_index"]) for r in result]==[("2026-08-31",0,0),("2026-08-25",0,1)]
